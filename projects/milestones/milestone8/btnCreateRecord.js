// CREATE NEW FORM RECORD for Milestone8

var webSeviceName = 'createRecordWebService';
            ...
            if (errorOnConexion) {
                ...
                if (wsSentResponse) {
                    if (wsSentSuccessResponse) {
                        message = formName + ' form record has been saved successfully.';

                        if (resp.meta.status == '200') {
                            if (resp.data[0] != undefined) {
                                if (resp.data[0] == 'Success') {
                                    //Saves formID to update form field values on updateRecordWebService.
                                    var formID = resp.data[2];
                                    VV.Form.SetFieldValue('formID', formID);
                                }
                            }
                        }
